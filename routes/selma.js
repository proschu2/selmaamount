...
const selma = async () => {
  ...
  return [amount.trim(), perc]
}

router.get('/', async function (req, res, ) {
  const selmaAmount = await selma();
  res.send(`${selmaAmount[0]} | ${selmaAmount[1]}`);
});
...
router.get('/add', async function (req, res, next) {
  ...
  try {
    ...
    const result = await client.query(`INSERT INTO SELMA(date,amount,percentage) VALUES (now(),${Number(selmaAmount[0])},${Number(selmaAmount[1])});`);
    const results = {
